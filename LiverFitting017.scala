...
object LiverFitting017 {
  ...
  def fitMesh(activeShapeModel : ActiveShapeModel,
    ...
    var generator = MixtureProposal.fromProposalsWithTransition(
      ...
      (0.03, shapeUpdateProposal),   //at 0.6, distance is 277.646, 0.2 279.0666, 0.9 276.7218
      (0.02, rotationUpdateProposal),//initial 0.2
      (0.02, translationUpdateProposal)
    )
    ...
    var samples = samplingIterator.drop(1000).take(20000).toIndexedSeq
    ...
    //val filename=new File("datasets/testset/fitting-result/liver-012_new.stl")
   // MeshIO.writeMesh(bestFit, filename)


    //val referenceMesh = MeshIO.readMesh(new java.io.File("datasets/validationset/fitting-result/liver-orig012.vtk")).get
    ...
    val discreteTargetImage1 = ImageIO.read3DScalarImageAsType[Float](new java.io.File("datasets/validationset/volume-ct/liver-017.nii")).get.map(_.toFloat)
    ...
    println("Distance is "+ distance)
  val filename=new File("datasets/testset/fitting-result/liver-017.stl")
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val discreteTargetImage = ImageIO.read3DScalarImageAsType[Float](new java.io.File("datasets/validationset/volume-ct/liver-017.nii")).get
