...
object LiverFitting019 {
  ...
  def fitMesh(activeShapeModel : ActiveShapeModel,
    ...
    var generator = MixtureProposal.fromProposalsWithTransition(
      (0, shapeUpdateProposal),   //at 0.6, distance is 277.646, 0.2 279.0666, 0.9 276.7218
      (0.4, rotationUpdateProposal),//initial 0.2
      (0.4, translationUpdateProposal),
      (0.0065, shapeUpdateProposal),   //at 0.6, distance is 277.646, 0.2 279.0666, 0.9 276.7218
    ...
    var samples = samplingIterator.drop(1000).take(10000).toIndexedSeq
    ...
    println(COM)
    //val referenceMesh = MeshIO.readMesh(new java.io.File("datasets/validationset/fitting-result/liver-orig014.vtk")).get
    ...
    val discreteTargetImage1 = ImageIO.read3DScalarImageAsType[Float](new java.io.File("datasets/validationset/volume-ct/liver-019.nii")).get.map(_.toFloat)
    ...
    case class ImageEvaluator(asm : ActiveShapeModel, preprocessedImage: PreprocessedImage)
      ...
      override def logValue(sample: Sample): Double = {
        ...
        //val currModelInstance = asm.instance(sample.parameters.modelCoefficients).transform(sample.poseTransformation)
    ...
    generator = MixtureProposal.fromProposalsWithTransition(
      (0.9, shapeUpdateProposal),   //at 0.6, distance is 277.646, 0.2 279.0666, 0.9 276.7218
      (0.2, rotationUpdateProposal),//initial 0.2
      (0.2, translationUpdateProposal)
    )
    ...
    samples = samplingIterator.drop(2).take(20).toIndexedSeq
    println(logger.acceptanceRatios())
    ...
    println("Distance is "+ distance)
    val filename=new File("datasets/testset/fitting-result/liver-019.stl")
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val discreteTargetImage = ImageIO.read3DScalarImageAsType[Float](new java.io.File("datasets/validationset/volume-ct/liver-019.nii")).get
